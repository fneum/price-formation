...
\begin{abstract}
  ...
  shares of wind and solar are high. However, given a certain capacity
  of storage, it is then unclear how this storage should bid in real
  markets where only dispatch is optimized. In this note we provide
  the answer by lifting storage constraints into the objective
  function via the Lagrangian, thus showing that the shadow prices
  from the original problem dictate the storage bids in the short-term
  model.
\end{abstract}
...
dispatch are always the same as the prices in short-term models that
only optimize dispatch. This is due to the fact that the value of lost
load (VOLL) screens out the capital costs from the long-term model, so
that only the VOLL and marginal costs of the generators appear in the
long-term prices.

This does NOT happen with storage in the model.

It was shown in \cite{Brown2020} that in long-term models storage make
back their costs exactly from the market prices (zero-profit rule).

But how exactly are prices formed given that prices no longer
correspond to marginal costs of generators? What do the storage units
bid?
...
Lift constraints up into objective function.

\subsection{Single node long-term equilibrium with storage}\label{sec:storage}
...
The demand balancing equation \eqref{eq:balance} is modified to:
\begin{equation}
